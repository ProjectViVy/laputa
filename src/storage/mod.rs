...
impl Layer1 {
    ...
    pub async fn generate(&self) -> String {
        let vs = match open_vector_storage(&self.config) {
            Ok(vs) => vs,
            Err(_) => return "## L1 — Could not connect to vector storage.".to_string(),
        };
...
mod tests {
    ...
    fn test_layer0_new_and_render() {
        ...
        let path_text = l0_default.path.to_string_lossy();
        assert!(path_text.contains(".mempalace"));
        assert!(path_text.contains("identity.txt"));
