...
mod tests {
    ...
    fn test_diary_public_functions() {
        ...
        let diary_db = temp_dir.path().join("diary.db");

        write_diary_at(&diary_db, "public-agent", "Public entry").unwrap();

        let entries = read_diary_at(&diary_db, "public-agent", 5).unwrap();
        assert_eq!(entries.len(), 1);
