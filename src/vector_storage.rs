...
use crate::storage::memory::{
    datetime_to_unix_timestamp, ensure_memory_schema, unix_timestamp_to_datetime, DEFAULT_HEAT_I32,
};
...
impl VectorStorage {
    ...
    pub fn add_memory(
        ...
        self.db.execute(
            "INSERT INTO memories (
             ...
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, 0, 5.0, ?8, 0, 0, 0)",
            params![
                text,
                wing,
                room,
    ...
    pub fn search_room(
        ...
        let sql = format!(
            "SELECT id, text_content, wing, room, source_file, valid_from, valid_to, last_accessed, access_count, importance_score, heat_i32, emotion_valence, emotion_arousal, is_archive_candidate
    ...
    pub fn get_memories(
        ...
        let (sql, params_dyn): (String, Vec<Box<dyn rusqlite::ToSql>>) = match (wing, room) {
            (Some(w), Some(r)) => (
                format!("SELECT id, text_content, wing, room, source_file, valid_from, valid_to, last_accessed, access_count, importance_score, heat_i32, emotion_valence, emotion_arousal, is_archive_candidate FROM memories WHERE wing = ?1 AND room = ?2 ORDER BY valid_from DESC LIMIT {limit}"),
                vec![Box::new(w.to_string()), Box::new(r.to_string())],
            ),
            (Some(w), None) => (
                format!("SELECT id, text_content, wing, room, source_file, valid_from, valid_to, last_accessed, access_count, importance_score, heat_i32, emotion_valence, emotion_arousal, is_archive_candidate FROM memories WHERE wing = ?1 ORDER BY valid_from DESC LIMIT {limit}"),
                vec![Box::new(w.to_string())],
            ),
            (None, Some(r)) => (
                format!("SELECT id, text_content, wing, room, source_file, valid_from, valid_to, last_accessed, access_count, importance_score, heat_i32, emotion_valence, emotion_arousal, is_archive_candidate FROM memories WHERE room = ?1 ORDER BY valid_from DESC LIMIT {limit}"),
                vec![Box::new(r.to_string())],
            ),
            (None, None) => (
                format!("SELECT id, text_content, wing, room, source_file, valid_from, valid_to, last_accessed, access_count, importance_score, heat_i32, emotion_valence, emotion_arousal, is_archive_candidate FROM memories ORDER BY valid_from DESC LIMIT {limit}"),
                vec![],
            ),
        };
        let mut stmt = self.db.prepare(&sql)?;
    ...
    pub fn get_memory_by_id(&self, id: i64) -> Result<MemoryRecord> {
        self.db.query_row(
            "SELECT id, text_content, wing, room, source_file, valid_from, valid_to, last_accessed, access_count, importance_score, heat_i32, emotion_valence, emotion_arousal, is_archive_candidate FROM memories WHERE id = ?1",
            params![id],
